...
- `CLAUDE_SMS_SERVER_URL` - Base URL for SMS server (e.g., https://sms.api.squidcorp.ink)
...
curl -X POST https://sms.api.squidcorp.ink/login/submit-with-setup \
  -H "Content-Type: application/json" \
  -d '{"phone":"+1234567890","setup_id":"test-123"}'

# Exchange for token
curl -X POST https://sms.api.squidcorp.ink/auth/exchange-by-setup \
  -H "Content-Type: application/json" \
  -d '{"setup_id":"test-123","pairing_code":"123456"}'

# Start session
curl -X POST https://sms.api.squidcorp.ink/sessions/start \
...
export const BLOCKING_PATTERNS =
  /permission denied|rate limit|fatal|crash|connection refused|unauthorized|authentication failed/i;
```
...
Default: `https://sms.api.squidcorp.ink`
